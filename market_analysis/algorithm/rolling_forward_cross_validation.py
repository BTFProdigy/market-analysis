...
from arima import SARIMAX
from market_analysis.algorithm import ModelConfiguration, ModelEvaluator
...
class RollingForwardCrossValidation:
    ...
    def validation(self, test_set_size, data):
        ...
        # predicted= pd.DataFrame(columns = data.columns)
        ...
        while (training_data_len < max_data):
            ...
            try:
                if num_of_predictions_ahead == 1:
                    self.train(configuration, training_data, num_of_predictions_ahead)
                elif prediction_counter % num_of_predictions_ahead == 0:
                    self.train(configuration, training_data, num_of_predictions_ahead)
